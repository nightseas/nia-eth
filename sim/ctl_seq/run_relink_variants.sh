...
declare -A ARM=(
  ...
  [B7]="-f Makefile.link_ctl confirm1"
  [B8]="-f Makefile.link_ctl wide"
  [B9]="-f Makefile.link_ctl SEED=2"
  [B10]="-f Makefile.link_ctl lint"

  [S1]="-f Makefile.wdt"
  [S2]="-f Makefile.wdt shortwin"
  [S3]="-f Makefile.wdt longwin"
  [S4]="-f Makefile.wdt SEED=2"
  [S5]="-f Makefile.wdt lint"
  [S6]="-f Makefile.exec"
  [S7]="-f Makefile.exec nopri"
  [S8]="-f Makefile.exec five"
  [S9]="-f Makefile.exec notmo"
  [S10]="-f Makefile.exec SEED=2"
  [S11]="-f Makefile.exec lint"

  [C1]="@../../sim/seam|!OPT_FAST=-O0 ./run_matrix_mac_dual.sh"
...
WAVEB="B1 B2 B3 B4 B5 B6 B7 B8 B9 B10"
WAVES="S1 S2 S3 S4 S5 S6 S7 S8 S9 S10 S11"
WAVEC="C1"
WAVEM="M1"
...

{
  ...
  echo "  RTL md5 dcmac_ctl_seq.sv        $(md5sum "$HERE/../../rtl/ctl/dcmac_ctl_seq.sv"         2>/dev/null | cut -d' ' -f1)"
  echo "  RTL md5 wdt.sv                $(md5sum "$HERE/../../rtl/ctl/wdt.sv"                2>/dev/null | cut -d' ' -f1)"
  echo "  RTL md5 dcmac_axil_exec.sv    $(md5sum "$HERE/../../rtl/ctl/dcmac_axil_exec.sv"     2>/dev/null | cut -d' ' -f1)"
  echo "  RTL md5 dcmac_link_csr.sv      $(md5sum "$HERE/../../rtl/ctl/dcmac_link_csr.sv" 2>/dev/null | cut -d' ' -f1)"
