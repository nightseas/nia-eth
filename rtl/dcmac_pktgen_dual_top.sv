// ---------------------------------------------------------------------------
// File        : dcmac_pktgen_dual_top.sv
// Description : The two client segmented instrument subsystem: one generator per client,
//               one control plane across both, and the register blocks they answer at.
...
module dcmac_pktgen_dual_top #(
  ...
  output wire [1:0]              ctl_gt_rx_done,

  input  wire [2*PKTGEN_AXIL_AW-1:0] pg_awaddr,
  input  wire [1:0]              pg_awvalid,
  output wire [1:0]              pg_awready,
  input  wire [63:0]             pg_wdata,
  input  wire [7:0]              pg_wstrb,
  input  wire [1:0]              pg_wvalid,
  output wire [1:0]              pg_wready,
  output wire [3:0]              pg_bresp,
  output wire [1:0]              pg_bvalid,
  input  wire [1:0]              pg_bready,
  input  wire [2*PKTGEN_AXIL_AW-1:0] pg_araddr,
  input  wire [1:0]              pg_arvalid,
  output wire [1:0]              pg_arready,
  output wire [63:0]             pg_rdata,
  output wire [3:0]              pg_rresp,
  output wire [1:0]              pg_rvalid,
  input  wire [1:0]              pg_rready
);
  ...
  assign ctl_gt_rx_done       = gt_rx_done_seg;
  ...
  for (q = 0; q < N_CLIENT; q++) begin : g_pktgen
    dcmac_seg_pktgen #(
      ...
      .s_axil_awaddr (pg_awaddr[q*PKTGEN_AXIL_AW +: PKTGEN_AXIL_AW]),
      .s_axil_awvalid(pg_awvalid[q]),
      .s_axil_awready(pg_awready[q]),
      .s_axil_wdata  (pg_wdata[q*32 +: 32]),
      .s_axil_wstrb  (pg_wstrb[q*4 +: 4]),
      .s_axil_wvalid (pg_wvalid[q]),
      .s_axil_wready (pg_wready[q]),
      .s_axil_bresp  (pg_bresp[q*2 +: 2]),
      .s_axil_bvalid (pg_bvalid[q]),
      .s_axil_bready (pg_bready[q]),
      .s_axil_araddr (pg_araddr[q*PKTGEN_AXIL_AW +: PKTGEN_AXIL_AW]),
      .s_axil_arvalid(pg_arvalid[q]),
      .s_axil_arready(pg_arready[q]),
      .s_axil_rdata  (pg_rdata[q*32 +: 32]),
      .s_axil_rresp  (pg_rresp[q*2 +: 2]),
      .s_axil_rvalid (pg_rvalid[q]),
      .s_axil_rready (pg_rready[q])
    );
  end
  endgenerate
